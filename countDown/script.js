...
let countDown = setInterval(function() {
    ...
    startedAt = GetURLParameter("start")
        ...
        .then(text => {
            if(text != window.location) { // Permalink no longer in clipboard
                ...
                if(permalink.classList.contains("flash")){
                    ...
                    if(permalink.textContent != "Click to copy your permalink to this countdown") {
    ...
    if(isNaN(distance) || countDownDate < startedAt) {
        ...
        if(startedAt < Date.now()) {
            ...
            document.getElementById("time_elapsed").innerHTML = `Time elapsed: ${elapsed}`;
            if(Date.now() > countDownDate){
                countDownElement.innerHTML = `Timer ended +${ms(Date.now() - countDownDate)}`
            }  
        }
    }
...
function textDate(date) {
    ...
    for (let i = 0; i < date.length; i++) {
        const x = date[i];
        if(x == 0 && names[i] != "s" && notNull == false) { continue; };
        date[i] = x + names[i]
        notNull = true
    }
...
function copy(text) {
    if(!navigator.clipboard) {return err}
    navigator.clipboard
      .writeText(text)
      .then(() => {
        // Popup "SUCCESSFUL"
...
function editTemp(text, id, duration, replace){
    ...
    if(element != null){
        ...
        setTimeout(() => {
            ...
            if(replace == true) { element.innerHTML = previous }
        }, duration);
    }
}